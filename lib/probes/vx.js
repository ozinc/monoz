...
function vx(props, callback) {
  return function(callback) {
    ...
    var options = {
      url: props.tokenUrl ? props.tokenUrl + props.channel : 'https://api.oz.com/v1/offering/oz/' + props.offering + '/token',
...
//    console.log(' Fetching token using '+ JSON.stringify(options));
    ...
    .spread(function (res, body) {
      ...
      //      console.log('    BODY' + JSON.stringify(body));
            ...
            if (props.vxType != 'vod' && props.channel != body.asset) {
      ...
      masterPlaylistUrl = replaceHost(masterPlaylistUrl, props.vxPlaylist);
...
//      console.log(' Fetching master Playlist using '+ masterPlaylist);
    ...
    .spread(function (res, body) {
      ...
      // Use a different host for special test cases
      segmentUrl = replaceHost(segmentUrl, props.vxSegment);

      // Identify keys that need to be requested
      var keyUrls = [];
      var n = 0;
...
//          console.log('Pushing key '+ keyUrl);
    ...
    .each(function (key) {
      ...
      .spread(function (res, body) {
        var diff = new Date() - before;
        console.log('  - fetched key ' + key.index + ' in ' + diff + ' ms  ('+ res.statusCode +' '+ res.headers['content-length'] +' bytes url: ' +  key.url + ')');
    ...
    .spread(function (res, body) {
      var diff = new Date() - before;
      console.log('  - fetched video segment in ' + diff + ' ms  ('+ res.statusCode +' url: ' + segmentUrl + ')');
      queueMetric(props, 'video segment', diff, res.statusCode);
    })
    .finally(function () {
      return callback();
    })
